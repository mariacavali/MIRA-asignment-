...
export class MiraEmailOutboxWorker {
  ...
  async processOne() { const job = await this.repository.claimDue(this.now(), 5 * 60_000); if (!job) return "idle" as const; const context = await this.resolveContext(job); const now = this.now(); if (!context || !context.invitationValid || context.shootCancelled || context.preparationCompleted || (context.shootEndsAt && now > context.shootEndsAt)) { await this.repository.markSuppressed(job.id, "not_deliverable"); return "suppressed" as const; } if (!context.clientEmail) { await this.repository.markFailed(job.id, "missing_recipient", new Date(now.getTime() + 60_000), this.maxAttempts); return "failed" as const; } try { await this.provider.send({ to: context.clientEmail, from: this.from, ...renderMilestone(job.milestoneId, context) } as TransactionalEmail); await this.repository.markSent(job.id, now); return "sent" as const; } catch { await this.repository.markFailed(job.id, "provider_delivery", new Date(now.getTime() + 60_000), this.maxAttempts); return "failed" as const; } }
}
