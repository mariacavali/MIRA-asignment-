...
export type EmailOutboxStatus = "pending" | "processing" | "sent" | "delivered" | "failed" | "suppressed" | "cancelled";
export type EmailOutboxJob = { id: string; shootId: number; invitationId: string; milestoneId: MiraClientEmailMilestoneId; scheduledAt: Date; status: EmailOutboxStatus; attemptCount: number; lastErrorCategory: string | null; idempotencyKey: string; providerMessageId: string | null; leaseUntil: Date | null; claimedAt: Date | null; sentAt: Date | null };

export interface EmailOutboxRepository {
  schedule(job: Omit<EmailOutboxJob, "id" | "status" | "attemptCount" | "lastErrorCategory" | "providerMessageId" | "leaseUntil" | "claimedAt" | "sentAt">): Promise<EmailOutboxJob>;
  claimDue(now: Date, leaseMs: number): Promise<EmailOutboxJob | null>;
  markSent(id: string, now: Date, providerMessageId?: string | null): Promise<void>;
  markDeliveredByMessageId(providerMessageId: string): Promise<void>;
...
export class InMemoryEmailOutboxRepository implements EmailOutboxRepository {
  ...
  private nextId = 1;
  async schedule(input: Omit<EmailOutboxJob, "id" | "status" | "attemptCount" | "lastErrorCategory" | "providerMessageId" | "leaseUntil" | "claimedAt" | "sentAt">) {
    ...
    const job: EmailOutboxJob = { ...input, id: String(this.nextId++), status: "pending", attemptCount: 0, lastErrorCategory: null, providerMessageId: null, leaseUntil: null, claimedAt: null, sentAt: null };
    this.jobs.push(job);
    return { ...job };
  }
  ...
  async markSent(id: string, now: Date, providerMessageId: string | null = null) { const job = this.jobs.find(item => item.id === id); if (job) { job.status = "sent"; job.sentAt = now; job.providerMessageId = providerMessageId; job.leaseUntil = null; } }
  async markDeliveredByMessageId(providerMessageId: string) { const job = this.jobs.find(item => item.providerMessageId === providerMessageId && item.status === "sent"); if (job) job.status = "delivered"; }
...
export async function recordImmediateInvitationAsSent(repository: EmailOutboxRepository, input: { invitationId: string; shootId: number; scheduledAt: Date; idempotencyKey: string; providerMessageId?: string | null }, sentAt = new Date()) {
  const job = await repository.schedule({ shootId: input.shootId, invitationId: input.invitationId, scheduledAt: input.scheduledAt, idempotencyKey: input.idempotencyKey, milestoneId: "shoot_room_invitation" });
  if (job.status !== "sent") await repository.markSent(job.id, sentAt, input.providerMessageId ?? null);
...
export class DrizzleEmailOutboxRepository implements EmailOutboxRepository {
  ...
  async markSent(id: string, now: Date, providerMessageId: string | null = null) { const db = await requireDb(); await db.update(miraEmailOutbox).set({ status: "sent", sentAt: now, providerMessageId, leaseUntil: null }).where(eq(miraEmailOutbox.id, Number(id))); }
  async markDeliveredByMessageId(providerMessageId: string) { const db = await requireDb(); await db.update(miraEmailOutbox).set({ status: "delivered" }).where(and(eq(miraEmailOutbox.providerMessageId, providerMessageId), eq(miraEmailOutbox.status, "sent"))); }
